...
public class AddUserFrame extends JInternalFrame implements ActionListener{
	...
	private JRadioButton userRole3 = new JRadioButton("管理者 	");
	...
	public void initPanel(){
		...
		if(sNo==0){
			...
			setTitle("修改用户界面");
			String user[] = UserLog.getUserBID(sNo);
		...
		loginContent1 = new JPanel(new GridLayout(1,2));
...
//		if(PencilMain.nowUser.u_role>=2){	//管理员加普通用户
//			grp.add(userRole2);
//			loginContent1.add(userRole2);
//		}
		if(PencilMain.nowUser.u_role==3){	//系统管理者
			grp.add(userRole3);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(btn==confirm){
			...
			if(faculty.equals("")||faculty==null){
			...
			if(uName.equals("")||userNumber.equals("")){
				...
				if(!Helper.matchPhone(userNumber)){
					JOptionPane.showMessageDialog(null, "用户账号为手机号，格式错误！");
					return;
				}
				try {
					int userRole;
					if(userRole1.isSelected())
						...
						userRole = 3;
					if(sNo==0){	//添加用户
						boolean result = UserLog.SelectUser(userNumber);
						if(result){
							JOptionPane.showMessageDialog(null, "用户账号已存在");
							return;
						}else{
						...
						boolean res = UserLog.updateUser(uName,userNumber,userRole,faculty);
						if(res){
