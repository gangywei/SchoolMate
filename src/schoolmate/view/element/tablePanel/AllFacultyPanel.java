...
import schoolmate.control.tableModel.FacultyModel;
...
import schoolmate.view.element.TabelPanel.RowListener;
...
public class AllFacultyPanel extends TabelPanel implements ActionListener{
	...
	public void initTable(){
		try {
			data = UserLog.dao("select * from faculty");
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		model = new FacultyModel(data);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(item==deleteItem){
			...
			if(PencilMain.dbControl){
				collectFrame.pencil.alterLabel(this,model.nowColumn,text,3);
			}else{
				JOptionPane.showMessageDialog(null, "正在导入数据，不允许该操作");
			}
		}
